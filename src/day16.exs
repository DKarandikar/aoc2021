defmodule Day16 do
  ...
  def solvePartA() do
    Day16.getLines |> hd |> Base.decode16!() |> packet() |> elem(2)
  end

  def solvePartB() do
    Day16.getLines |> hd |> Base.decode16!() |> packet() |> elem(1)
  end
  ...
  def packetType0(bits, packet_list, version_acc, final_remainder) do
    ...
    packetType0(remainder, packet_list ++ [output], version_acc + version, final_remainder)
  end
  ...
  def packetType1(bits, packet_list, n, version_acc) do
    {remainder, output, version} = packet(bits)
    packetType1(remainder, packet_list ++ [output], n - 1, version_acc + version)
  end
